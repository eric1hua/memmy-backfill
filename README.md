...
- **重嵌旧向量** — 把旧维度的向量重新嵌入到当前模型的维度。
  ...
  只处理**现在就该修**的那些：原文向量（`vec`）随时可以重嵌，但摘要向量（`vec_summary`）
  如果摘要还是占位符，就会被跳过。原因是维度不匹配的向量检索时直接被排除在外，
  此刻它们是**不可见**的；提前把占位符重嵌成新维度，等于让一批内容是「摘要排队中」
  的垃圾向量重新变得可见，跟真实结果抢排名。等摘要补上时会一步到位，
  先写内容再换维度，也省一次嵌入费用。
...
闸门脚本直接调用面板的 `quota()`，两边共用同一套判定——否则很容易出现
「面板显示会拦截，定时任务照跑」这种界面与实际不一致的情况。
